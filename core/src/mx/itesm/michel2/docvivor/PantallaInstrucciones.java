...
public class PantallaInstrucciones extends Pantalla {
    ...
    public void render(float delta) {
        ...
        batch.draw(imgpausa,ANCHO/2,0);
        batch.draw(pause,(ANCHO/2)+imgjump.getWidth(),0);
        batch.draw(imgjump,0,0+128);
        batch.draw(jump,0+imgjump.getWidth(),0+128);
        batch.draw(imgder,ANCHO/2,0+256);
        batch.draw(right,(ANCHO/2)+imgjump.getWidth(),0+256);
        batch.draw(imgleft,0,0+384);
        batch.draw(left,0+imgjump.getWidth(),0+384);
